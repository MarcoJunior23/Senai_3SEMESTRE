...
export const mapApiResponseToEnvironments = 
    (apiResponse: ApiResponse<EnvironmentResponse,Device>):
        Array<Environment> => {
        
         return apiResponse.items.map(item=>{
        //api|ambientes=environments|map=for
            if(item.fields){
                const environment = new Environment();
                environment.name = item.fields.name;
                environment.id = item.sys.id;
                environment.devices = item.fields.devices?.map(
                 sysDevice=>{
    ...
    export class NewEnvironment {
        name: ApiAtribute<string> = new ApiAtribute("");   
    }
